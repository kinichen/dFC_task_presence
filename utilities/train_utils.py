...
def cross_validation_control(X, y, subj_label, train_config, train_one_fold, 
                             test_one_fold, model_name, dataset_name, seed=42):
    ...
    for trainval_idx, test_idx in outer_kf.split(X, y, groups=subj_label):
        ...
        if fold == 1 and learning_plot: # save losses for fold 1
            ...
            np.save(os.path.join(save_dir, f"inner_train_losses_{dataset_name}_{date_str}.npy"),
                    np.round(np.array(total_inner_train_losses, dtype=float), decimals=5))  
                # Shape: (num_hyperparams_combos*num_inner_folds, num_epochs)
            np.save(os.path.join(save_dir, f"val_losses_{dataset_name}_{date_str}.npy"),
                    np.round(np.array(total_val_losses, dtype=float), decimals=5))
            np.save(os.path.join(save_dir, f"outer_train_losses_{dataset_name}_{date_str}.npy"),
                    np.round(np.array(outer_train_losses, dtype=float), decimals=5))
                # Shape: (num_epochs,)
            np.save(os.path.join(save_dir, f"test_losses_{dataset_name}_{date_str}.npy"),
    ...
    os.makedirs(save_dir, exist_ok=True)
    np.save(os.path.join(save_dir, f"acc_{dataset_name}_{date_str}.npy"), 
            np.round(np.array(acc_scores, dtype=float), decimals=4))
    np.save(os.path.join(save_dir, f"auc_{dataset_name}_{date_str}.npy"), 
